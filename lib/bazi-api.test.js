...
test('getGeJu lets revealed secondary qi lead a four-birth month when main qi is not exposed', () => {
  const result = baziModule.getGeJu({
    year: ['丙', '子'],
    month: ['辛', '寅'],
    day: ['乙', '酉'],
  ...
  assert.equal(result.basis, '主格候选');
  assert.equal(result.monthMainGan, '丙');
  assert.equal(result.tenGod, '伤');
  assert.equal(result.geju, '伤官格');
  assert.equal(result.patternCandidates.some((item) => item.type === 'birth_month_revealed_hidden' && item.stem === '丙'), true);
});
...
test('getChengGe only admits 金水伤官用官 for winter metal with 官露伤藏、官根、财印并透', () => {
  const result = baziModule.getChengGe({
    geju: '伤官格',
    basis: '月令本气',
    dayGan: '庚',
  ...
  assert.equal(result.yongShenTenGod, '官');
  assert.match(result.chengGeReason, /冬月庚辛金日主/);
});

test('壬癸日主不因属水而误判为金水伤官喜官', () => {
  const result = baziModule.getChengGe({
    geju: '伤官格',
    basis: '月令本气',
    dayGan: '壬',
  ...
  assert.notEqual(result.chengGe, '伤官喜官格');
  assert.equal(result.patternEffects.methodTags.includes('金水伤官'), false);
});
...
test('hasLatestEngineCache invalidates pre-image-analysis engine caches', () => {
  ...
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.28' } }), false);
  assert.equal(hasLatestEngineCache({ bazi_detail: { engine_version: '1.8.29' } }), true);
});
