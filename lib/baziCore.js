...
const BAZI_ENGINE_VERSION = '1.8.29';
...
// 《子平真诠·论伤官》：“伤官用官者……金水独宜，然要财印为辅，
// 不可伤官并透”，并以“藏癸露丁，戊甲为辅，官又得禄”为成例。
// 因此这里只承认冬月庚辛日主的水伤官；官须透而有根，伤官须藏，
// 财印也须见于天干。壬癸日主本身属水，不属于“金水伤官”。
function _isQualifiedMetalWaterShangGuanOfficer({ dayGan, monthZhi, states, finance, seal }) {
    return ['庚', '辛'].includes(dayGan)
...
function _resolvePatternCandidates({ dayGan, monthZhi, allStems, allBranches, monthHiddenGans, monthMainGan, rawMonthMainGan, basis, monthMergeInfo }) {
    ...
    // 《子平真诠·论用神变化》明确说月令变化“不必四库”。四生月本气不透、
    // 而长生或余气透干时，透出者可以作主；本气与余气同透时则保留本格，
    // 将透出者列为兼格候选，不能见一余气便粗暴覆盖。
    const isBirthMonth = ['寅', '申', '巳', '亥'].includes(monthZhi);
    if (isBirthMonth) {
        const rawMonthMainIsVisible = _isStemTouGan(rawMonthMainGan, allStems);
        const visibleAuxiliaryStems = monthHiddenGans.slice(1).filter(stem => stem && _isStemTouGan(stem, allStems));
        monthHiddenGans.slice(1).forEach((stem, auxiliaryIndex) => {
            if (!stem || !_isStemTouGan(stem, allStems)) return;
            const tenGod = tenGodMap[stem];
            if (!tenGod || tenGod === '比' || tenGod === '劫') return;
            pushCandidate(_buildPatternCandidate({
                type: 'birth_month_revealed_hidden',
                stem,
                tenGod,
                priority: rawMonthMainIsVisible || visibleAuxiliaryStems.length > 1 ? 72 - auxiliaryIndex : 84 - auxiliaryIndex,
                evidence: rawMonthMainIsVisible
                    ? [`四生月${monthZhi}中${stem}透干`, `本气${rawMonthMainGan}亦透，${_formatGeJuByTenGod(tenGod)}只作兼格候选`]
                    : visibleAuxiliaryStems.length > 1
                        ? [`四生月${monthZhi}本气${rawMonthMainGan}不透`, `余气${visibleAuxiliaryStems.join('、')}并透，须按纯杂制化细辨，${_formatGeJuByTenGod(tenGod)}先作兼格候选`]
                        : [`四生月${monthZhi}本气${rawMonthMainGan}不透`, `${stem}由月令藏干透出作主`]
            }));
        });
    }

    // 合会会改变月令作用，但成局后仍须结合透干、纯杂与全局成败判断。
    // 此处把变化写入候选，不单凭“见合”覆盖本格；后续成格层再判断能否落实。
    if (monthMergeInfo) {
        const mergeStems = _ELEMENT_TO_STEMS[monthMergeInfo.resultElement] || [];
        const visibleMergeStem = mergeStems.find(stem => _isStemTouGan(stem, allStems));
        const mergeStem = visibleMergeStem || monthMergeInfo.resultGan;
        const mergeTenGod = tenGodMap[mergeStem];
        if (mergeStem && mergeTenGod && _GAN_TO_ELEMENT[monthMainGan] !== monthMergeInfo.resultElement) {
            pushCandidate(_buildPatternCandidate({
                type: 'month_branch_transformation',
                stem: mergeStem,
                tenGod: mergeTenGod,
                priority: 76,
                evidence: [
                    `${monthZhi}参与${monthMergeInfo.mergeType}化${monthMergeInfo.resultElement}`,
                    visibleMergeStem
                        ? `${visibleMergeStem}透干承接合会之气，列入变化候选`
                        : '合会成局但化神未透，保留为变化候选'
                ]
            }));
        }
...
function getChengGe(baziInfo) {
    ...
    if (geju === '伤官格') {
        ...
        // 金水伤官用官：仅限冬金，且须官露伤藏、官有根、财印并透为辅。
        if (_isQualifiedMetalWaterShangGuanOfficer({
            dayGan,
            monthZhi: baziInfo.monthZhi,
            states,
            finance,
            seal
        })) {
            return setResult('官', '伤官喜官格', '成格', '冬月庚辛金日主，水伤官藏而不透，官星透而有根，财印并透为辅，取官调候。', '伤', states.guan.stem, {
                protectedShens: ['官', '财', '才', '印', '枭'],
                assistantShens: ['财', '才', '印', '枭'],
                methodTags: ['金水伤官', '官露伤藏', '财印为辅', '官星有根']
            });
        }
        if (_isAnyShangGuanJianGuan(dayGan, states)) return setResult('伤', '伤官格', '败格', '伤官见官', '', states.shang.stem);
        // 伤官带杀，用伤制杀
...
const BaziEngine = {
    ...
    getGeJu: function (bazi) {
        ...
        if (!monthMergeInfo && isGraveyardMonth) {
            // 杂气墓库月可直接取透干余气立格；四生月的余气透干由候选层按纯杂另判。
